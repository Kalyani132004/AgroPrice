...
def historical_prices_view(request):
    ...
    history = (
        price_service.history(crop_name, days=days)
        if crop_name else []
    )

    stats = (
        price_service.thirty_day_stats(crop_name)
        if crop_name else {}
    )

    return render(
        request,
        "prices/historical_prices.html",
        {
...
def compare_markets_view(request):
    ...
    comparisons = (
        price_service.compare_markets(crop_name)
        if crop_name else []
    )

    return render(
        request,
        "prices/compare_markets.html",
        {
            "crops": crop_service.list_all(),
            "selected_crop": crop_name,
...
def upload_csv_view(request):
    if request.method == "POST" and request.FILES.get("csv_file"):
        ...
        if result.success_count:
            messages.success(
                request,
                f"{result.success_count} price record(s) imported successfully."
            )
